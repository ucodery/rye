...
fn dent_tokens(#[case] source: &str, #[case] total_indents: usize) {
    ...
    } in tokens.iter()
    {
        if *token_type == TokenType::INDENT {
            ...
            assert!(
                indents_found <= total_indents,
                "{} INDENTs were found when only {} were expected",
                indents_found,
                total_indents
            );
        };
        if *token_type == TokenType::DEDENT {
            ...
            assert!(
                dedents_found <= indents_found,
                "{} DEDENTs were found when only {} INDENTs had previously been found",
                dedents_found,
                indents_found
            );
        };
    ...
    assert_eq!(
        indents_found, total_indents,
        "Not enough INDENTs were found. Was expecting {} got {}",
        total_indents, indents_found
    );
    assert_eq!(
        indents_found,
        dedents_found,
        "Not every INDENT had a DEDENT. Found {} more INDENTs",
        (indents_found - dedents_found)
    );
}
